...
3. Выберите репозиторий `dryagungleb14-wq/ai_coach2_dasha`
...
6. `ffmpeg` будет установлен автоматически через `nixpacks.toml`
...
- `HUGGINGFACE_TOKEN` - токен от Hugging Face (опционально, для диарризации)
...
4. **Whisper модель**: При первом анализе Whisper скачает модель `base` (~150MB), это может занять время
5. **Порты**: Railway автоматически устанавливает PORT, бэкенд использует его
...
- Если Whisper не работает, убедитесь что `ffmpeg` установлен (установка через nixpacks.toml)
